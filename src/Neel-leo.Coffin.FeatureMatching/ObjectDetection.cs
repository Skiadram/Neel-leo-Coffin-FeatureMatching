...
namespace Neel_leo.Coffin.FeatureMaching
{
    ...
    {
        public IList<ObjectDetectionResult> DetectObjectInScenes(byte[] objectImageData,
            IList<byte[]> imagesSceneData)
        {
            var myList = new List<ObjectDetectionResult>();
            foreach (var imscene in imagesSceneData)
            {
                
                    using var imgObject = Mat.FromImageData(objectImageData,
                        ImreadModes.Color);
                    using var imgScene = Mat.FromImageData(imscene,
                        ImreadModes.Color);
                    ...
                    var odr = new ObjectDetectionResult()
                    {
